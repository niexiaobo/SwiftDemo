...
class HZHMatchPhotoDetailCell: UICollectionViewCell {
    
    var delegate: JFPhotoDetailCellDelegate?
    
    var model: HZHMatchInfoModel? {
        didSet {
    ...
    private func layoutImageView(image: UIImage) {
        ...
        if size.height < (ScreenHeight) {
            // 短图, 居中显示
            let offestY = (ScreenHeight - size.height) * 0.5
...
extension HZHMatchPhotoDetailCell: UIScrollViewDelegate {
