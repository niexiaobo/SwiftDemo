...
class HZHRollingcycleView: UIView, UIScrollViewDelegate {
    /*********************************** Property ****************************************/
    //MARK:- Property
    ...
    var delegate: RollingcycleDelegate?
    ...
    private func setUpCircleView() {
        ...
        //添加点击事件
        let imageTap = UITapGestureRecognizer(target: self, action: #selector(HZHRollingcycleView.imageTapAction(_:)))
        ...
        //设置计时器
        self.timer = NSTimer.scheduledTimerWithTimeInterval(TimeInterval, target: self, selector: #selector(HZHRollingcycleView.timerAction), userInfo: nil, repeats: true)
    }
    
    //MARK: 设置图片
    private func setScrollViewOfImage(){
        let urlImage = NSURL(string: self.imageArray[self.indexOfCurrentImage])
        print("urlImage=====" , self.imageArray[self.indexOfCurrentImage])
        if urlImage != nil {
            self.currentImageView.sd_setImageWithURL(urlImage!, placeholderImage: UIImage.init(named: "zhanwei1"))
        }
        let nexturlImage = NSURL(string: self.imageArray[self.getNextImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)])
        if nexturlImage != nil {
            self.nextImageView.sd_setImageWithURL(nexturlImage!, placeholderImage: UIImage.init(named: "zhanwei1"))
        }
        let lasturlImage = NSURL(string: self.imageArray[self.getLastImageIndex(indexOfCurrentImage: self.indexOfCurrentImage)])
        if lasturlImage != nil {
            self.lastImageView.sd_setImageWithURL(lasturlImage!, placeholderImage: UIImage.init(named: "zhanwei1"))
        }
...
@objc protocol RollingcycleDelegate {
    /**
     *  点击图片的代理方法
     *
